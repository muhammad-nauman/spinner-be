...
@section('title') Edit Machine @endsection
@section('css')
...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <h1>Edit Machine</h1>
            <form class="form-horizontal" method="POST" action="{{ route('machines.update', ['machine' => $machine->id]) }}">

                {{ csrf_field() }}
                {{ method_field('PUT') }}
                <div class="form-group @error('name') has-error @enderror"><label class="col-lg-2 control-label">Name</label>
                    <div class="col-lg-7">
                        <input type="text" name="name" placeholder="Name of Machine" class="form-control" value="{{ $machine->name }}">
                        @error('name')
                ...
                <div class="form-group @error('location') has-error @enderror"><label class="col-lg-2 control-label">Location</label>
                    <div class="col-lg-7">
                        <input type="text" name="location" placeholder="Location of the Machine" class="form-control" value="{{ $machine->location }}">
                        @error('location')
                        <span class="help-block text-red m-b-none">{{ $message }}</span>
                        @enderror
                    </div>
